...
GLOBAL_CONN = sqlite3.connect(DB_FILE, check_same_thread=False, timeout=3)
GLOBAL_CONN.execute("PRAGMA journal_mode=WAL;")
GLOBAL_CONN.execute("PRAGMA synchronous=NORMAL;")  # optional performance boost
GLOBAL_CONN.commit()
...
def update_masseira_daily():
    ...
    with DB_LOCK:
        ...
        for dev in DEVICES:
            ...
            start_ts = _get_meta_value(meta_key)
            if not start_ts:
